...
def run(
    ...
    # Initialize
    person_counter = 0
    person_list: Dict[str, Tuple[int, int]] = {}
            ...
            if det is not None and len(det):
                ...
                if len(outputs[i]) > 0:
                    for j, (output, conf) in enumerate(zip(outputs[i], det[:, 4])):
                        ...
                        # bboxの中心座標
                        bbox_center = (bbox_left + bbox_w / 2, bbox_top + bbox_h / 2)

                        # 境界線の2頂点の座標
                        im_h, im_w, depth = im0.shape
                        im_point1 = (im_w / 2, 0)
                        im_point2 = (im_w / 2, im_h)

                        if str(id) in person_list:
                            prev_point = person_list.get(str(id))
                            if crossing_detection(
                                bbox_center, prev_point, im_point1, im_point2
                            ):
                                if enter_exit_detection(
                                    im_point1, im_point2, bbox_center
                                ):
                                    person_counter += 1
                                else:
                                    person_counter -= 1
                        person_list[str(id)] = bbox_center
